...
#pragma PERSISTENT(USSSWLIB_SAPH_DATAERR_callback)
#pragma PERSISTENT(USSSWLIB_SAPH_TMFTO_callback)
#pragma PERSISTENT(USSSWLIB_SAPH_SEQDN_callback)
#pragma PERSISTENT(USSSWLIB_SAPH_PNGDN_callback)
#pragma PERSISTENT(USSSWLIB_UUPS_PREQIG_callback)
#pragma PERSISTENT(USSSWLIB_UUPS_PTMOUT_callback)
#pragma PERSISTENT(USSSWLIB_UUPS_STPBYDB_callback)
#pragma PERSISTENT(USSSWLIB_HSPLL_PLLunlock_callback)
#pragma PERSISTENT(USSSWLIB_SDHS_OVF_callback)
#pragma PERSISTENT(USSSWLIB_SDHS_ACQDONE_callback)
#pragma PERSISTENT(USSSWLIB_SDHS_SSTRG_callback)
#pragma PERSISTENT(USSSWLIB_SDHS_DTRDY_callback)
#pragma PERSISTENT(USSSWLIB_SDHS_WINHI_callback)
#pragma PERSISTENT(USSSWLIB_SDHS_WINLO_callback)
#endif
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SAPH_DATAERR_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SAPH_TMFTO_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SAPH_SEQDN_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SAPH_PNGDN_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_UUPS_PREQIG_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_UUPS_PTMOUT_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_UUPS_STPBYDB_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_HSPLL_PLLunlock_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SDHS_OVF_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SDHS_ACQDONE_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SDHS_SSTRG_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SDHS_DTRDY_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SDHS_WINHI_callback) (void)=0;
#if defined (__IAR_SYSTEMS_ICC__)
__persistent
#endif
void (*USSSWLIB_SDHS_WINLO_callback) (void)=0;

volatile uint16_t USSSWLIB_USS_measurementPeriod;


#if((USSSWLIB_ENABLE_EXTERNAL_CIRCUITRY_CONTROL == true) &&\
     (__EVM430_ID__ == 0x43) && (defined(__AFE_EXT_3v3__) || defined(__AFE_EXT_5v0__)))
void USSLIB_HAL_initGPIO(void)
{
    // Initialize RxEn to be controlled by T0.1 OUT
    USSSWLIB_HAL_AFE_RXEN_PORT &= ~(USSSWLIB_HAL_AFE_RXEN_PIN);
    USSSWLIB_HAL_AFE_RXEN_PORT_DIR |= (USSSWLIB_HAL_AFE_RXEN_PIN);

    // Initialize RxSel1 and RxSel2 gpios (clear gpio and configure as output)
    USSSWLIB_HAL_AFE_RX_SELx_PORT &= ~(USSSWLIB_HAL_AFE_RX_SEL1_PIN
            | USSSWLIB_HAL_AFE_RX_SEL2_PIN);
    USSSWLIB_HAL_AFE_RX_SELx_PORT_DIR |= (USSSWLIB_HAL_AFE_RX_SEL1_PIN
            | USSSWLIB_HAL_AFE_RX_SEL2_PIN);

    // Initialize RxPwr gpios (set gpio and configure as output)
    // RxPwr is expected to be on all the time
    USSSWLIB_HAL_AFE_RXPWR_PORT |= (USSSWLIB_HAL_AFE_RXPWR_PIN);
    USSSWLIB_HAL_AFE_RXPWR_PORT_DIR |= (USSSWLIB_HAL_AFE_RXPWR_PIN);

#if (__EVM430_VER__ == 0x20)
    // Initialize TxPwr gpios (clear gpio and configure as output)
    USSSWLIB_HAL_AFE_TXPWR_PORT &= ~(USSSWLIB_HAL_AFE_TXPWR_PIN);
    USSSWLIB_HAL_AFE_TXPWR_PORT_DIR |= (USSSWLIB_HAL_AFE_TXPWR_PIN);
...
void USSLIB_HAL_configGPIOForCapture(USSLIB_HAL_capture_mode mode)
{
    switch(mode)
    {
    case USSLIB_HAL_capture_mode_ups:
        USSSWLIB_HAL_AFE_RX_SELx_PORT |= (USSSWLIB_HAL_AFE_RX_SEL1_PIN |
                USSSWLIB_HAL_AFE_RX_SEL2_PIN);

        break;
    case USSLIB_HAL_capture_mode_dns:
        USSSWLIB_HAL_AFE_RX_SELx_PORT &= ~(USSSWLIB_HAL_AFE_RX_SEL1_PIN |
                USSSWLIB_HAL_AFE_RX_SEL2_PIN);
        break;
    }
...
{
    // Timer uses SMCLK. It's used in LPM0 during acquisition only
    HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCTL) = (TASSEL__SMCLK | ID__1 | MC__STOP | TACLR);

    // TAx.1 used to trigger RxEn
    HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCCR1) = RxEnTimerCounts;

    // TAx.2 used to trigger ASQ
    HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCCR2) = ASQTimerCounts;

}

void USSLIB_HAL_startRxEnTimer(void)
{
    // Enable CCR2 interrupt to start ASQ
    HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCCTL2) = CCIE;
    // Disable CCR1 interrupt by default, enabled in ISR after CCR2
    HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCCTL1) = 0x00;
    // Start Timer in continuous mode
    HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCTL) |= ( TACLR | MC__CONTINOUS);
}
...
#if (__EVM430_VER__ == 0x20)
    // Turn OFF TX POWER after acquisition
    USSSWLIB_HAL_AFE_TXPWR_PORT &= ~(USSSWLIB_HAL_AFE_TXPWR_PIN);

#if defined(__AFE_EXT_5v0__)
    // Turn OFF TX 5V EN after acquisition
    USSSWLIB_HAL_AFE_TX5VEN_PORT &= ~(USSSWLIB_HAL_AFE_TX5VEN_PIN);
#endif
...
__interrupt void USSLIB_HAL_TIMER_RXEN_INT(void)
{
                           ...
                           TAIV__TAIFG))
    {
        ...
        case TAIV__TACCR2:           // CCR2, start ASQ, restart timer to trigger RxEN
            ...
            HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCCTL2) = 0x00;
            // Enable CCR1 interrupt to enable RxEN
            HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCCTL1) = CCIE;
            // Restart timer
            HARDWAREWREG16(USSSWLIB_HAL_RXEN_TIMER_BASE_ADDRESS + OFS_TAxCTL) |= ( TACLR | MC__CONTINOUS);
            // Trigger ASQ
...
#pragma vector= USSLIB_HAL_TIMER_CCR0_VECTOR
__interrupt void USSLIB_HAL_TIMER_INT(void)
{
    uint16_t counterReg; 
    USSSWLIB_USS_interrupt_status |= (USS_TIMER_EXPIRED_INTERRUPT);
    HARDWAREWREG16(USSSWLIB_TIMER_BASE_ADDRESS + OFS_TAxCCTL0) &= ~(CCIFG);
    counterReg = HARDWAREWREG16(USSSWLIB_TIMER_BASE_ADDRESS + OFS_TAxR);
    // Reload timer with measurement rate
    HARDWAREWREG16(USSSWLIB_TIMER_BASE_ADDRESS+OFS_TAxCCR0) =
                counterReg + USSSWLIB_USS_measurementPeriod;
    LPM4_EXIT;
}

#pragma vector= USSLIB_HAL_TIMER_CCR1_VECTOR
__interrupt void USSLIB_HAL_TIMER_INT_CCRn(void)
{
    /* Any access, read or write, of the TBIV register automatically resets the
       highest "pending" interrupt flag. */
    switch(__even_in_range(HARDWAREWREG16(USSSWLIB_TIMER_BASE_ADDRESS + OFS_TAxIV),TAIV__TAIFG))
    {
    case  0: break;                            // No interrupt
    // CCR1 and CCR2 must have same / similar number of cycles
    case  2:                                   // CCR1
...
__interrupt void ISR_SDHS(void)
{
    switch (__even_in_range(SDHSIIDX,IIDX_6))
    {
        case IIDX_1: // SDHS Data OVerflow
            USSSWLIB_USS_interrupt_status |=USS_SDHS_OVF_INTERRUPT;
            if (USSSWLIB_SDHS_OVF_callback)
            {
                USSSWLIB_SDHS_OVF_callback();
            }
            break;
        case IIDX_2: // SDHS Acquisition Done
            USSSWLIB_USS_interrupt_status |=USS_SDHS_ACQDONE_INTERRUPT;
            if (USSSWLIB_SDHS_ACQDONE_callback)
            {
                USSSWLIB_SDHS_ACQDONE_callback();
            }
            break;
        case IIDX_3:    // SDHS Start conversion Trigger
            USSSWLIB_USS_interrupt_status |=USS_SDHS_SSTRG_INTERRUPT;
            if (USSSWLIB_SDHS_SSTRG_callback)
            {
                USSSWLIB_SDHS_SSTRG_callback();
            }
            break;
        case IIDX_4: // SDHS Data Ready
            USSSWLIB_USS_interrupt_status |=USS_SDHS_DTRDY_INTERRUPT;
            if (USSSWLIB_SDHS_DTRDY_callback)
            {
                USSSWLIB_SDHS_DTRDY_callback();
            }
            break;
        case IIDX_5: // SDHS Window High
            USSSWLIB_USS_interrupt_status |=USS_SDHS_WINHI_INTERRUPT;
            if (USSSWLIB_SDHS_WINHI_callback)
            {
                USSSWLIB_SDHS_WINHI_callback();
            }
            break;
        case IIDX_6: // SDHS Window Low
            USSSWLIB_USS_interrupt_status |=USS_SDHS_WINLO_INTERRUPT;
            if (USSSWLIB_SDHS_WINLO_callback)
            {
...
__interrupt void ISR_UUPS(void)
{
    switch (__even_in_range(UUPSIIDX, IIDX_3))
    {
        case IIDX_1: // UUPS Power Up Timem Out
            USSSWLIB_USS_interrupt_status |= USS_UUPS_PWR_UP_TM_OUT_INTERRUPT;
            if (USSSWLIB_UUPS_PTMOUT_callback)
            {
                USSSWLIB_UUPS_PTMOUT_callback();
            }
            break;
        case IIDX_2: // UUPS Power request Ignored
            USSSWLIB_USS_interrupt_status |= USS_UUPS_PWR_REQ_IGN_INTERRUPT;
            if (USSSWLIB_UUPS_PREQIG_callback)
            {
                USSSWLIB_UUPS_PREQIG_callback();
            }
            break;
        case IIDX_3: // USS has been interrupted by debug mode
            USSSWLIB_USS_interrupt_status |= USS_UUPS_STOPPED_BY_DEBUGGER;
            if (USSSWLIB_UUPS_STPBYDB_callback)
            {
...
__interrupt void ISR_SAPH(void)
{
    switch (__even_in_range(SAPHIIDX,IIDX_4))
    {
        case IIDX_1: // Data Error Abort
            USSSWLIB_USS_interrupt_status |= USS_SAPH_DATA_ERR_ABORT_INTERRUPT;
            if (USSSWLIB_SAPH_DATAERR_callback)
            {
                USSSWLIB_SAPH_DATAERR_callback();
            }
            break;
        case IIDX_2:    // Time mark 4 time out
            USSSWLIB_USS_interrupt_status |= USS_SAPH_TMF_TM_OUT_INTERRUPT;
            if (USSSWLIB_SAPH_TMFTO_callback)
            {
        ...
        case IIDX_3: // Sequence acquisition done
            ...
            if (USSSWLIB_SAPH_SEQDN_callback)
            {
                USSSWLIB_SAPH_SEQDN_callback();
            }
            break;
        case IIDX_4: // Ping transmit done
            USSSWLIB_USS_interrupt_status |= USS_SAPH_PING_DN_INTERRUPT;
            if (USSSWLIB_SAPH_PNGDN_callback)
            {
...
__interrupt void USS_LEA_ISR(void)
#elif defined(__GNUC__) && (__MSP430__)
void __attribute__ ((interrupt(LEA_VECTOR))) USS_LEA_ISR(void)
#endif
{
    // Save the interrupt flags and clear interrupts
    uint32_t flags = LEAIFG;
    LEAIFG |= flags;
    USS_LEA_IFG = flags;
